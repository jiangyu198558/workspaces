...
autoLogin = (req, res, next) ->
  ...
  db.users.findOne({token: token, expiredTime: {$gt: Date.now()}},
    (err, user) ->
      return next(err) if err
...
logout = (req, res, next) ->
  db.users.update({_id: req.userInfo._id}, {$set: {token: '', expiredTime: Date.now()}},
    (err, numReplaced) ->
      return next(err) if err
      return next(new Error('注销失败，请重试！')) if numReplaced is 0
      res.json(true)
  )
